/* This component is rendered after the user clicks on Create License Application and handles all the core react-flow module features */
...
const HomeComponent = () => {
  ...
  async function onSave() {
    ...
    let allElements = await Promise.all(
      myElements.map((e) => {
        if (!isEdge(e)) {
          /* On click of save check if globalNode checkbox was selected for that node. If yes then add it to the globalNodes state variable */
          if (e.data.isGlobal) {
            globalNodes.push(e);
          }
          myData.push({ ...e.data, id: e.id });
        }
        /* Store the update position as well so the next time the user views this flow, the positions are maintained. */
        return { ...e, position: positions[e.id] };
      })
    );
    /* On save we send three pieces of info to the backend. 1) TemplateData (Dynamic data to be used in forms or preview) 2) DiagramFlowData (Nodes positoning, styling and the order in which they are connected) 3) Array of all template sections (if any)*/
    let resp = await fetch(
      "/api/templateElements/templateData/" + params.templateId,
      {
        method: "PUT",
    ...
    if (resp.status !== 200) {
      ...
      /* If there are globalNodes then send the globalNodes to another table which handles all the globalNodes. */
  ...
  function onChange(evt, ind = "1") {
    setMyElements((els) =>
      els.map((e) => {
        ...
        /* Since options data is in the form of comma seprated values we split it to form an array cause that is how the options are stored in the backend */
        if (name === "options") value = value.split(",");

        /* Since the names mentioned below are checkboxes, instead of value we check whether they are checked or not(boolean) */
        if (
          name === "isGlobal" ||
          name === "isMandatory" ||
          name === "isDependent"
  ...
  function addNode(sourceIndex) {
    ...
    /* This is a module function which is used to add an edge between the newly created node and the one that created it. */
    onConnect(myEdge);
  }
  ...
  return (
    ...
    >
      {/* Main Library component to use nodes and links. Checkout the documentation of react-flow (https://github.com/wbkd/react-flow) for more details on module specific helper functions, componenets, examples and API's. */}
      <ReactFlow
        deleteKeyCode={46}
        elements={myElements}
      ...
      >
        ...
        />
        <Controls />
        {/* To add buttons in the react flow always keep position absolute and zIndex 4. Rest can be adjusted according to your needs. */}
        <Button
          color="primary"
